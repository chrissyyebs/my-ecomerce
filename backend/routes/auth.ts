 ...
 * Permanently deletes a user account after random confirmation word validation
 */
router.post('/delete-account', authLimiter, async (req, res, next) => {
  try {
    ...
    const { email, confirmationWord, expectedWord } = parsed.data;

    // Verify the random confirmation word matches
    ...
    console.log(`[Auth] Account permanently deleted via confirmation word: ${email}`);
