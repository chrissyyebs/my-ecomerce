...
export const MEMORY_PRODUCTS: any[] = [];
...
router.get('/', async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.warn('[GET /api/products] Supabase query offline/unavailable');
    }

    const formatted = (data || []).map((p: any) => ({
      ...p,
      categoryName: p.category?.name || 'Bags',
      image: p.images?.[0]?.public_url || p.image || '',
    }));

    const combined = [...MEMORY_PRODUCTS, ...formatted];
    const unique = Array.from(new Map(combined.map((p) => [p.id, p])).values());

    res.json({ products: unique });
  } catch (err) {
    res.json({ products: MEMORY_PRODUCTS });
  }
});
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      // Resolve category_id from categoryName if not provided or not a valid UUID
      if (!req.body.category_id && req.body.categoryName) {
        const { data: cat } = await supabase
          .from('categories')
          .select('id')
          .ilike('name', req.body.categoryName)
          .limit(1)
          .single();
        if (cat?.id) {
      ...
      await logAction(userId, 'create', 'product', product.id, { name: product.name });
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      await logAction(userId, 'update', 'product', id, parsed.data);
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      await logAction(userId, 'delete', 'product', id, { name: data.name });
