...
export async function sendMessage(
  ...
  try {
    ...
    console.warn(`[TelegramService] Network call failed for chat ${chatId}:`, err?.message || err);
  }
}
...
export async function notifyNewOrder(order: {
  ...
  const text = [
    ...
    `<b>Total:</b> GH₵${order.total_amount.toFixed(2)}`,
  ].join('\n');
...
export async function notifySalesSummary(
  ...
  const topList =
    summary.topProducts.length > 0
      ? summary.topProducts
          .map(
            (p, i) =>
              `  ${i + 1}. ${p.name} — GH₵${p.revenue.toFixed(2)} (${p.quantity} sold)`
          )
          .join('\n')
  ...
  const text = [
    ...
    '',
    `<b>Revenue:</b> GH₵${summary.totalRevenue.toFixed(2)}`,
...
export async function notifyStuckOrders(
  ...
  const items = orders
    .map((o) => {
      ...
      return `  • <code>${o.id.slice(0, 8)}</code> — ${o.customer_name} — GH₵${Number(o.total_amount).toFixed(2)} (${mins}min ago)`;
    })
    .join('\n');
...
export async function sendInlineButtons(
  ...
  try {
    ...
    console.warn('[TelegramService] sendInlineButtons network warning:', err?.message || err);
  }
}
...
export async function answerCallbackQuery(
  ...
  try {
    ...
    console.warn('[TelegramService] answerCallbackQuery network warning:', err?.message || err);
  }
}
